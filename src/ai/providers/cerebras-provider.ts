...
export class CerebrasProvider implements AIProvider {
  ...
  async checkConnection(budget?: RequestBudget): Promise<{ success: boolean; error?: string; httpStatus?: number | null }> {
    ...
    try {
      ...
      const hasModel = data.objects?.some((m: any) => m.id === this.model);
      if (!hasModel) {
...
storyScore: 0,
        postQualityScore: 0,
        category: '',
        reason: `Cerebras error: ${lastError}`,
            shouldPost: false,
            verifiedFacts: [],
            primaryPost: { type: '', text: '' },
            alternativePosts: [],
...
storyScore: 0,
        postQualityScore: 0,
        category: '',
        reason: `Cerebras error after retries: ${lastError}`,
      shouldPost: false,
      verifiedFacts: [],
      primaryPost: { type: '', text: '' },
      alternativePosts: [],
...
Evaluate this story and return ONLY valid JSON.
  ...
  private parseResponse(text: string): EvaluationResult {
    ...
    try {
      ...
      return {
        ...
        postQualityScore: 0,
        category: '',
        reason: 'Failed to parse Cerebras response',
        shouldPost: false,
        verifiedFacts: [],
        primaryPost: { type: '', text: '' },
        alternativePosts: [],
